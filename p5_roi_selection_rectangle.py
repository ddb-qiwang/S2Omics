...
def main():
    ...
    for i in range(n_images):
        ...
        cluster_image_shape_list.append(np.shape(cluster_image))
    ...
    concat_cluster_image_shape = [np.sum(cluster_image_shape_list,axis=0)[0]+(n_images-1)*padding_size,np.max(cluster_image_shape_list,axis=0)[1]]
    concat_cluster_image = -1*np.ones(concat_cluster_image_shape)
    concat_cluster_image_mask = np.full(concat_cluster_image_shape, False)
    concat_cluster_image_rgb = 255*np.ones([concat_cluster_image_shape[0],concat_cluster_image_shape[1],3])
    starting_x = 0
    for i in range(n_images):
        cluster_image_shape = cluster_image_shape_list[i]
        cluster_image = cluster_image_list[i]
        concat_cluster_image[starting_x:starting_x+cluster_image_shape[0],0:cluster_image_shape[1]] = cluster_image
        starting_x += cluster_image_shape[0] + padding_size
    ...
    length = np.max(concat_cluster_image_shape)//100
    plt_figsize = (concat_cluster_image_shape[1]//100,concat_cluster_image_shape[0]//100)
    if dpi*length > np.power(2,16):
        reduce_ratio = np.power(2,16)/(dpi*length)
        plt_figsize = ((concat_cluster_image_shape[1]*reduce_ratio)//100,(concat_cluster_image_shape[0]*reduce_ratio)//100)
    if plt_figsize[0] < 10:
        plt_figsize = (10,int(10*(concat_cluster_image_shape[0]/concat_cluster_image_shape[1])))
